...
async def show_main_commands(update: Update, context: Application.Context) -> None:
    ...
    keyboard = [
        ...
        [InlineKeyboardButton("م2: اوامر الاعدادات", callback_data="cmd_m2")],
        ...
        [InlineKeyboardButton("🔙 العودة للقائمة الرئيسية", callback_data="main_menu")] # This button is for consistency, it will just re-show the main menu
    ]
...
async def get_other_rank(update: Update, context: Application.Context) -> None:
    ...
    if not target_user:
        await update.message.reply_text(
            f"المستخدم {target_first_name or target_username} (ID: {target_user_id}) غير مسجل في قاعدة بيانات البوت بعد."
        )
        return
